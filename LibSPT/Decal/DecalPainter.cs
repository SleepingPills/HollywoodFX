...
public class DecalPainter
{
    ...
    private readonly Dictionary<Material, DeferredDecalRenderer.GClass1026> _dictionary0;
    private readonly Dictionary<Camera, DeferredDecalRenderer.Class718> _dictionary2;
    
    public DecalPainter(DeferredDecalRenderer renderer)
    {
        _renderer = renderer;
        var traverse = Traverse.Create(_renderer);
        _dictionary0 = traverse.Field("dictionary_0").GetValue<Dictionary<Material, DeferredDecalRenderer.GClass1026>>();
        _dictionary2 = traverse.Field("dictionary_2").GetValue<Dictionary<Camera, DeferredDecalRenderer.Class718>>();
    }
